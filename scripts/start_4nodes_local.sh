...
pkill -9 -f "wand-node" || true
pkill -9 -f "cmd/node/main.go" || true
rm -f /tmp/node*.log || true
sleep 1

echo "Building node binary..."
go build -o wand-node cmd/node/main.go
...
./wand-node node1 8001 > /tmp/node1.log 2>&1 &
./wand-node node2 8002 > /tmp/node2.log 2>&1 &
./wand-node node3 8003 > /tmp/node3.log 2>&1 &
./wand-node node4 8004 > /tmp/node4.log 2>&1 &
